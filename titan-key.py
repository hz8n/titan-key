#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Ghost Key Forge — titan-key v1.1.6
...
BANNER = f"""{C}{BOLD}  ⬓⬓⬓  Ghost Key Forge  ⬓⬓⬓{RST}
{C}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RST}
  {W}titan-key • Crypto • v1.1.6 • Design by al3rab • Full English{RST}
{M}  RSA/EC keygen + public key exporter for lab{RST}
"""

def get_info():
    try: ip=socket.gethostbyname(socket.gethostname())
    except: ip="127.0.0.1"
    return {"host":socket.gethostname(),"ip":ip,"os":platform.system(),"time":datetime.datetime.now().isoformat(),"tool":"titan-key","title":"Ghost Key Forge","category":"Crypto","designer":"al3rab","lang":"English"}

def main():
    p=argparse.ArgumentParser(description="Ghost Key Forge - RSA/EC keygen + public key exporter for lab | Design by al3rab")
    ...
    args=p.parse_args()
    print(BANNER)
    print(f"{G}[✓] Legendary advanced engine armed{RST}  {GR}titan-key v1.1.6 • Design by al3rab{RST}")
    ...
    if not is_private and not args.allow_public:
        ...
        sys.exit(1)
    data=get_info()
    data.update({"target":args.target,"findings":["RSA/EC keygen + public key exporter for lab - advanced finding 1","Legendary terminal check passed"],"risk":"LOW","legendary":True,"advanced":True})
